...
Výchozí interval je 30 minut. Dá se změnit v **Nastavení → Zařízení a služby →
PODA data → Konfigurovat** (rozsah 0,5 – 24 hodin).
...
2. Tag verze musí odpovídat `"version"` v `custom_components/poda_data/manifest.json`
   (např. `v0.2.2`). **HACS detekuje aktualizace podle tagovaných releasů**,
